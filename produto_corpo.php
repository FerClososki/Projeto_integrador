...
session_start();
include "conexao.php";
$sql = "SELECT * FROM produtos WHERE categoria = 'corpo'";
...
<head>
    ...
    <title>Produtos para Corpo</title>
    <style>
        body {
...
<body>
    ...
    <div class="top-bar">
        ...
        <h2 class="cabelo">Produtos para Corpo</h2>
        <a href="carrinho.php">
    ...
    <div class="produtos-container">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<a href="produto_corpo.php?id=' . intval($row['id']) . '">';
                $imagem = htmlspecialchars($row['imagem']);
                if (!empty($imagem)) {
                    echo '<img src="' . $imagem . '" alt="' . htmlspecialchars($row['nome']) . '" class="imagem-com-borda">';
                } else {
                    echo '<p>Imagem não disponível</p>';
                }

                echo '<p><strong>' . htmlspecialchars($row['nome']) . '</strong><br>R$' . number_format($row['preco'], 2, ',', '.') . '</p>';
                echo '</a>';

                echo '<form method="POST" action="adicionar_carrinho.php" class="quantidade">';
                echo '<input type="hidden" name="produto_id" value="' . intval($row['id']) . '">';
